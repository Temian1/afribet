    ...
    return (
        <label className="block">
            <span className="relative block">
                ...
                <input
                    {...props}
                    className="w-full rounded-xl border border-[var(--pf-border)] bg-slate-50 py-3 pl-11 pr-11 text-sm text-slate-950 outline-none transition placeholder:text-slate-400 focus:border-purple-l focus:ring-4 focus:ring-purple/10 dark:bg-white/[.04] dark:text-slate-100"
                />
...
export default function AuthModal({ open, initialMode = 'login', onClose }) {
    ...
    return (
        ...
        <div className="fixed inset-0 z-[1500] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-[420px] overflow-hidden rounded-2xl border border-[var(--pf-border)] bg-white p-6 shadow-2xl dark:bg-ink-2 sm:p-7">
                ...
                <button onClick={close} className="absolute right-4 top-4 z-10 text-slate-500 transition hover:text-slate-950" type="button" aria-label="Close"><X size={20} /></button>
                ...
                <div className="relative">
                    ...
                    {(mode === 'login' || mode === 'register') && (
                        <div className="mb-5 flex items-start gap-2 rounded-xl border border-[var(--pf-accent)]/25 bg-[var(--pf-accent)]/10 px-3 py-2.5 text-xs leading-relaxed text-[var(--pf-accent)] dark:text-[#8fffd6]">
                            <span className="mt-0.5 size-2 shrink-0 rounded-full bg-[var(--pf-accent)] shadow-[0_0_10px_#39f5ad]" />
                            <span><strong>Frontend demo:</strong> use any email and password. No real account or credentials are sent to a server.</span>
                        </div>
                    )}

                    {(mode === 'login' || mode === 'register') && (
                        <div className="mb-5 grid grid-cols-2 gap-1 rounded-xl border border-[var(--pf-border)] bg-slate-100 p-1 dark:bg-white/[.04]">
                    ...
                    <form onSubmit={submit} className="space-y-3">
                        ...
                        {mode === 'login' && (
                            <div className="flex items-center justify-between text-xs">
                                <button type="button" onClick={() => setRemember((value) => !value)} className="inline-flex items-center gap-2 text-slate-500">
                                    <span className={`flex h-4 w-4 items-center justify-center rounded border ${remember ? 'border-purple bg-purple text-[var(--pf-text)]' : 'border-[var(--pf-border)]'}`}>{remember && <Check size={12} />}</span>
                                    Remember me
                                </button>
                        ...
                        <button type="submit" disabled={busy} className="mt-4 w-full rounded-lg bg-gradient-to-br from-purple to-purple-d px-4 py-3 font-heading text-sm font-bold uppercase tracking-wider text-[var(--pf-text)] transition hover:from-purple-l hover:to-purple">
                    ...
                    {(mode === 'login' || mode === 'register') && (
                        <>
                            ...
                            <button onClick={google} disabled={busy} className="flex w-full items-center justify-center gap-2.5 rounded-lg border border-[var(--pf-border)] bg-slate-50 py-3 text-sm font-semibold text-slate-900 transition hover:border-purple-l/50 dark:bg-white/[.04] dark:text-slate-100" type="button">
