...
function AvatarPicker({ onClose }) {
    ...
    return (
        ...
        <div className="fixed inset-0 z-[1600] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-[420px] rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl dark:border-white/10 dark:bg-ink-2">
                ...
                <div className="mb-4 flex items-center gap-2">
                    <input value={seed} onChange={(e) => setSeed(e.target.value)} className="min-w-0 flex-1 rounded-lg border border-slate-200 bg-slate-50 px-3 py-2 text-sm outline-none focus:border-purple-l dark:border-white/10 dark:bg-white/[.04]" />
                    <button onClick={() => setSeed(Math.random().toString(36).slice(2, 9))} className="rounded-lg border border-slate-200 p-2 text-slate-600 dark:border-white/10 dark:text-slate-300" type="button"><Shuffle size={17} /></button>
                </div>
                <div className="grid grid-cols-4 gap-3">
                    {options.map((url, index) => {
                        const active = selected === url;
                        return (
                            <button key={url} onClick={() => setSelected(url)} className={`relative aspect-square rounded-xl border-2 p-1 transition ${active ? 'border-purple-l shadow-lg shadow-purple/20' : 'border-transparent hover:border-slate-300 dark:hover:border-white/20'}`} type="button">
                ...
                <div className="mt-5 flex gap-2">
                    <button onClick={onClose} className="flex-1 rounded-lg border border-slate-200 px-4 py-3 font-heading text-sm font-bold uppercase tracking-wider dark:border-white/10" type="button">Cancel</button>
...
export default function UserMenu() {
    ...
    return (
        <div className="relative" ref={ref}>
            <button onClick={() => setOpen((value) => !value)} className="flex items-center gap-2 rounded-full border border-slate-300 bg-white py-1 pl-1 pr-3 transition hover:border-purple-l/50 dark:border-white/10 dark:bg-white/[.04]" type="button">
            ...
            {open && (
                <div className="absolute right-0 mt-2 w-60 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl shadow-slate-900/15 dark:border-white/10 dark:bg-ink-2 dark:shadow-black/50">
                    ...
                    <div className="border-t border-slate-200 p-1.5 dark:border-white/10">
